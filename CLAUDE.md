...
**Asynchronous data belongs to the request that produced it.** Keep the payload and its request key together, adopt a mutation's response only when its captured origin still matches the current selection, and never treat a failed lookup as an empty result. `docs/frontend/api-and-cache.md` has the full rule and the regression matrix.
