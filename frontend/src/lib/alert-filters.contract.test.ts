...
/**
 * The system-vs-financial classification is one set written twice: the backend's
 * SYSTEM_NOTIFICATION_TYPES (the dismiss-matching endpoint restricts its UPDATE
 * on it) and the frontend mirror in `types/budget.ts` (the panel's category
 * filter and the local removal after a delete-all read it). If the two drift,
 * delete-all removes rows the user's filter never showed -- or leaves rows it
 * did. Nothing compiles the layers against each other, so this reads the backend
 * entity and compares member for member, the same trade as
 * `ai-query-budgets.contract.test.ts`.
 *
 * The backend entity moved to `notification-center/` with migration 172, when
 * `budget_alerts` became `notifications`; the frontend names are still the alert
 * ones and follow in their own pass.
 */
