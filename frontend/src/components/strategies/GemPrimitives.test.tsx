...
describe('GemPrimitives', () => {
  it('names a card section and renders its header extras', () => {
    render(
      <GemCard title="Current signal" hint="Explains the signal" headerRight={<span>RISK-ON</span>}>
        <p>body</p>
      </GemCard>,
    );
    expect(screen.getByRole('region', { name: /Current signal/ })).toBeInTheDocument();
  ...
  it('renders an untitled card body', () => {
    ...
    expect(screen.getByRole('region', { name: 'Bare card' })).toBeInTheDocument();
  });
  ...
  it('renders a known value and falls back for an unknown one', () => {
    const { rerender } = render(
      <GemValue value={12}>{(value) => <span>value is {value}</span>}</GemValue>,
    );
    expect(screen.getByText(/value is 12/)).toBeInTheDocument();
    rerender(<GemValue value={null}>{(value) => <span>value is {value}</span>}</GemValue>);
    expect(screen.getByText('Not available')).toBeInTheDocument();
  });
  ...
  it('draws only the track when every segment is empty', () => {
    ...
    expect(
      screen.getByRole('img', { name: 'No allocation' }).querySelectorAll('circle'),
    ).toHaveLength(1);
  });
