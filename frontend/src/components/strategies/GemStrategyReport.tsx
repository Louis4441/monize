...
export function GemStrategyReport() {
  ...
  const { data, isLoading, error, reload } = useReportData<GemStrategyReportData>(
    () => gemStrategyApi.getReport(range, strategyId),
    [range, strategyId, reloadKey],
  );
  ...
  const handleMarkExecuted = useCallback(async () => {
    ...
    setIsSaving(true);
    try {
      await gemStrategyApi.markExecuted(signalId, range);
  ...
  /**
   * Scenario operations all return the report for whichever scenario the user
   * should land on, so the id is taken from the response rather than assumed --
   * deleting one has to land somewhere, and the server decides where.
   */
  const handleCreateScenario = useCallback(
    async (name: string) => {
      ...
      try {
        ...
        setStrategyId(report.strategy.id);
        setTab('settings');
  ...
  const handleDeleteScenario = useCallback(
    async (id: string) => {
      setIsSaving(true);
      try {
        const report = await gemStrategyApi.deleteStrategy(id, range);
        setStrategyId(report.strategy.id === 'gem' ? undefined : report.strategy.id);
        setTab('overview');
  ...
  /**
   * A saved configuration can change the signal itself (a newly assigned role
   * makes the strategy evaluable), so re-read rather than patching state.
   */
