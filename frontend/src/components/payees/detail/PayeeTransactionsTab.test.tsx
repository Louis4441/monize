...
import { PayeeTransactionsTab } from './PayeeTransactionsTab';
...
const categoryLabelMap = new Map([['cat-1', 'Food: Coffee']]);
...
describe('PayeeTransactionsTab', () => {
  beforeEach(() => {
    mockGetAll.mockReset();
    mockGetAll.mockResolvedValue({ data: [transaction()], pagination: page(1) });
  });

  it('fetches every transaction for the payee, paginated', async () => {
    await renderTab();
  ...
  it('passes the hierarchical category labels through to the rows', async () => {
    await renderTab();
    expect(screen.getByTestId('label-map')).toHaveTextContent('Food: Coffee');
  });
