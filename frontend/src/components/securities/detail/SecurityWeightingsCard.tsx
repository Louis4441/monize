...
interface SecurityWeightingsCardProps {
  title: string;
 ...
 *
 * Renders nothing but its empty state when the data is absent. A security with
 * no sector data is not a security with zero-length bars.
 */
export function SecurityWeightingsCard({
  title,
  slices,
  emptyMessage,
  remainderLabel,
}: SecurityWeightingsCardProps) {
  ...
  const rows = useMemo(() => {
    ...
    // Largest first: the question this card answers is "what is it mostly".
