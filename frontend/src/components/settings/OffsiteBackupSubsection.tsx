...
import {
  ...
  BackupOffsiteUpload,
  OFFSITE_UPLOADS_PAGE_SIZE,
  UpdateBackupOffsiteSettingsData,
} from '@/lib/backupApi';
import { getErrorMessage } from '@/lib/errors';
import { OffsiteUploadsTable } from './OffsiteUploadsTable';
...
/**
 * A user's own off-machine copies of their completed automatic backups.
 *
 ...
 * typed one.
 */
export function OffsiteBackupSubsection() {
  const t = useTranslations('settings.backupRestore.offsite');
  ...
  const [uploads, setUploads] = useState<BackupOffsiteUpload[] | null>(null);
  ...
  const loadUploads = useCallback(async () => {
    ...
    try {
      setUploads(await backupApi.listOffsiteUploads(OFFSITE_UPLOADS_PAGE_SIZE));
    } catch (error) {
      setUploads(null);
      setUploadsError(getErrorMessage(error, t('uploads.loadFailed')));
    } finally {
    ...
    <div className="mb-6 pb-6 border-b border-gray-200 dark:border-gray-700">
      ...
      {!isLoading && view && (
        <>
          ...
          {isDemoMode && (
            <p role="note" className="mt-4 text-sm text-amber-700 dark:text-amber-300">
              {t('demoNote')}
            </p>
          )}
              ...
              <div className="mt-4 space-y-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <Input
                    ...
                    disabled={isDemoMode}
                    placeholder={
                      view.s3AccessKeyIdSet ? t('s3.storedPlaceholder') : undefined
                    }
