...
describe("AutoBackupSection", () => {
  ...
  describe("when the capability cannot be read", () => {
    it("fails closed: a disabled schedule cannot be armed while the capability is unknown", async () => {
      // A failed capability lookup is not permission to arm. Until the
      // deployment says storage is available, arming and Run Now stay disabled
      // for a schedule that is not already on (patch 0002, fail closed).
      (
        backupApi.getAutoBackupSettings as ReturnType<typeof vi.fn>
      ).mockResolvedValue({
        ...defaultSettings,
