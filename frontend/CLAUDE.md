...
**Not estimating has a price, and the place to pay it is the seed, not the history.** `deriveCurrentInstallment` returns `principal + interest` of the last regular row, so a loan booking its interest outside the app now yields a figure under one period's interest -- and `generateLoanSchedule` refuses such a seed outright, taking the payoff date and remaining interest with it. `buildLoanProjectionInput` therefore falls back to the stored contractual `paymentAmount` when the derived installment cannot cover `firstPeriodInterest`; when neither does there is no projection, and both figures read as unknown. Do not restore the estimate to keep a projection alive, and do not pick the largest number to hand -- that dresses a refusal up as a decision about the loan.
...
`ScheduledTransactionLoanService` carries the same absence predicate (`s.categoryId && !s.transferAccountId`) for the templates it recalculates. It is materially safer there — it reads back the canonical shape it wrote itself — but it is the same idea, and if it ever reads a user-edited template it needs the same provenance rule.
