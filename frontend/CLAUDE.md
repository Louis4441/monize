...
**A quick-fill copies what a row says, never where it lives.** The transaction form's Recent (history) button lists recents deduped *across accounts*, so the chosen row usually belongs to somewhere other than the account the modal was opened on. `handleQuickFill` fills payee, category, amount, description, tags and split lines and leaves `accountId` alone -- moving the entry to the source's account silently changes which ledger the user is writing to. `currencyCode` is not copied either: it is derived from the selected account by the account effect in `TransactionForm.tsx`, and a copied code would denominate the amount in a currency the account does not hold (the same "currency comes from the account, not the request" rule the backend enforces with `assertTransactionCurrencyMatchesAccount`). `TransactionForm.test.tsx`'s quick-fill block pins both.
