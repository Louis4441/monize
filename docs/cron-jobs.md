...
| `exchange-rate.service` | `5 17 * * 1-5` (America/New_York) | 5:05 PM ET weekdays | Fetch exchange rates (staggered after price refresh). Held by one replica per tick with `FetchSyncService.withLease("exchange-rates", 15 min)`; the `onModuleInit` startup fetch takes the same lease, so a rollout of N pods fetches once. A cost control, not a correctness one -- the rate upserts are idempotent -- so a lost lease is a debug line and the expiry alone hands the job back to the next tick |
...
| `security-price.service` | `0 17 * * 1-5` (America/New_York) | 5 PM ET weekdays | Fetch security prices, then settle the day: re-read each symbol's recent daily bars and overwrite the provisional intraday quotes with the provider's official OHLCV and adjusted close. Held by one replica per tick with `FetchSyncService.withLease("security-prices", 30 min)` -- the settlement and the snapshot recalc ride inside the lease because they are what the fetch is for. A cost control; the price upserts are idempotent and a lost lease never blocks the next tick |
| `market-index.service` | `10 17 * * 1-5` (America/New_York) | 5:10 PM ET weekdays | Fetch market index closes for the benchmark overlay (staggered after the price and FX refreshes). Held by one replica per tick with `FetchSyncService.withLease("market-indexes", 20 min)`, and the `onApplicationBootstrap` warm-up takes the same lease -- a rollout is where 24 indexes x up to 11 yearly chunks per pod is least welcome. The per-index `respectCooldown` in `market_index_sync` is a different question (how often ONE index is worth re-asking for) and is unchanged |
...
| `portfolio-movement-alert.service` | `40 17 * * 1-5` (America/New_York) | 5:40 PM ET weekdays | After the price refreshes, notify each opted-in user (a `move_alert_percent` threshold) when their investment-account value moved beyond the threshold, measuring `MV(today) - MV(baseline) - externalFlow` so a deposit does not fire and a dividend is return, not a loss. Each day's flow is converted at that day's rate, not the run day's (INV-PORTMOVE-007). Withheld and not re-baselined when the value or the flow is incomplete -- a subtotal is not a total. A close that arrives late restates the baseline rather than being reported as the period's move, and a carried close never withholds (INV-PORTMOVE-008). The alert names its period (both boundary dates) and its components. One row per day via the dedupe key. `withSystemContext` to enumerate opted-in users, `withUserContext` per user |
...
| `relay-sweeper.service` | `*/5 * * * *` | Every 5 minutes | Collect the AI relay's dead rows: expire a turn nobody claimed, and one an agent claimed or answered and nobody came back for, once the grace `post_response` itself enforces has run out; delete a turn that has been terminal for another grace period; drop confirmation cards past `expires_at`. Nothing the relay does depends on this run -- every statement in `AiRelayService` compares against `expires_at` in the database's clock, so a dead turn is already unclaimable and unanswerable -- it only keeps four tables from being append-only, and takes the bytes of an attachment nobody claimed with them (the blob is a cascading child row, so the whole sweep stays inside PostgreSQL). Idempotent by predicate, so every replica firing it touches the same rows and the losers touch none; deleting late is safe, which is why each cutoff is a stored timestamp plus an interval and never a clock the process holds |
...
| `auth-state-sweeper.service` | `0 4 * * *` | Daily 4 AM | Delete expired rate-limit counters (`auth_attempt_counters`), spent one-shot claims (`single_use_tokens`) and expired HTTP throttle counters (`http_throttle_counters`, written only in `CLUSTER_MODE=multi`). No table's correctness depends on this run -- an expired counter is reset in place by the incrementing statement, a claim against an expired token row is still a loss, and a throttle row past its window is reset by its next hit -- so the sweep only keeps three append-only tables from growing. The throttle delete spares a row whose window has passed but whose block has not, since that row is still serving a refusal. Idempotent across replicas: every predicate is a stored expiry, never a clock this process holds |
...
| `updates.service` | `0 0-23/12 * * *` | Every 12 hours | Refresh the latest-release metadata for the What's New digest into `update_check_state`, one row for the deployment. The conditional upsert that stamps `checked_at` **is** the claim, so replicas ticking together make one GitHub request rather than N -- the unauthenticated rate limit is per IP and shared by every replica behind one egress address. The startup refresh takes the same window, so a rollout and a restart inside it ask nothing. A failed check still holds the window and keeps the last known version, so the banner says "could not check" rather than "nothing to install" |
| `attachment-orphan-sweeper.service` | `0 0-23/1 * * *` | Hourly | Delete attachment bytes whose metadata is gone, and objects left by an abandoned upload; retains a swept upload intent for its late-write quarantine (no-op for the `database` provider). Sweeps each addressable backend in turn, not only the bound one: after a storage-provider switch the objects to reclaim are in the backend being moved away from, and a tombstone naming a backend this deployment cannot address is left alone because the record is the only remaining pointer to those bytes |
| `attachment-storage-migrator.service` | `0 50 * * * *` | Hourly at :50 | Move attachment bytes into the backend `ATTACHMENT_STORAGE_PROVIDER` now names, after an operator changed it; the `onApplicationBootstrap` pass is the one that normally does the work and this is its retry, costing one `LIMIT 1` over `transaction_attachments` when nothing is outside the active backend. Held by one replica per **batch** with `FetchSyncService.withLease("attachment-relocation", 20 min)` -- a cost control, because correctness is per row: each attachment is copied, read back, flipped and tombstoned inside one transaction that re-reads the row `FOR UPDATE` first, so a second replica writes nothing a first did not. `withSystemContext` for the keyset scan over rows whose `storage_provider` is not the active one, `withUserContext` per attachment. Switched off with `ATTACHMENT_STORAGE_MIGRATE_ON_SWITCH=false`, which says so once in the log rather than silently |
