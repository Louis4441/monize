...
| `notification-reminder.service` | `* * * * *` | Every minute | Fire due repeating/one-time reminders: stop any whose source was dismissed, then a single conditional `UPDATE ... RETURNING` claims each due row (advancing `next_fire_at` in the same statement, so a second replica's blocked UPDATE re-checks and skips it -- claimed exactly once), then re-emit each through the write door under the owner's context. `next_fire_at` is set to now + interval so a missed run fires once, never a catch-up burst |
