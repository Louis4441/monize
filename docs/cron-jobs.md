...
| `system-alert-monitor.service` | `*/15 * * * *` | Every 15 minutes | Raise an in-app SMTP_FAILURE admin alert when this replica's email sends are failing; reads a per-replica in-memory snapshot, and the `idx_budget_alerts_dedupe` unique index makes the row at most once per day across replicas |
