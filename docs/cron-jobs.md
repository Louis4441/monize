...
| `notification-reminder-cron.service` | `* * * * *` | Every minute | Fire due repeating/one-time reminders: stop any whose source was dismissed or deleted, then a single conditional `UPDATE ... RETURNING` claims each due row (advancing `next_fire_at` in the same statement, so a second replica's blocked UPDATE re-checks and skips it -- claimed exactly once), then re-emit each through the dispatch seam (`NotificationDispatchService.notify`: in-app row, then push/email per the matrix and throttle) under the owner's context, dismissing the previous nag of the same reminder in the same transaction. `next_fire_at` is set to now + interval so a missed run fires once, never a catch-up burst |
