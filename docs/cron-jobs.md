...
| `relay-sweeper.service` | `*/5 * * * *` | Every 5 minutes | Collect the AI relay's dead rows: expire a turn nobody claimed, and one an agent claimed or answered and nobody came back for, once the grace `post_response` itself enforces has run out; delete a turn that has been terminal for another grace period; drop confirmation cards past `expires_at`. Nothing the relay does depends on this run -- every statement in `AiRelayService` compares against `expires_at` in the database's clock, so a dead turn is already unclaimable and unanswerable -- it only keeps three tables from being append-only. Idempotent by predicate, so every replica firing it touches the same rows and the losers touch none; deleting late is safe, which is why each cutoff is a stored timestamp plus an interval and never a clock the process holds |
