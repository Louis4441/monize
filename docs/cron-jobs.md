...
| `auth-state-sweeper.service` | `0 4 * * *` | Daily 4 AM | Delete expired rate-limit counters (`auth_attempt_counters`) and spent one-shot claims (`single_use_tokens`). Neither table's correctness depends on this run -- an expired counter is reset in place by the incrementing statement and a claim against an expired token row is still a loss -- so the sweep only keeps two append-only tables from growing. Idempotent across replicas: the predicate is the stored expiry, never a clock this process holds |
