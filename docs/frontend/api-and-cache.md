...
The second instance was the category filter: a "select every income category" control and "Select All" both put a few hundred UUIDs into `categoryIds`, which four requests and the page URL then carried (~11 KB, over Node's 16 KB header budget once cookies are added; over an nginx ingress's 8 KB before that), and localStorage replayed the list on every later visit. The category filter names a type with the pseudo-ids `income` / `expense`, which the server expands beside `uncategorized` / `transfer` (`backend/src/common/category-tree.util.ts`), and `canonicalizeCategoryFilter` (`lib/categoryUtils.ts`) collapses any selection covering a whole type into its pseudo-id before the hook exposes it, so no reader can enumerate one. `SPECIAL_CATEGORY_FILTER_IDS` and `useCategoryFilterLabels` are the one list and its labels.
...
- **Client address:** `X-Forwarded-For` is replaced, never passed through, with the one address `assertedClientAddress` (`src/lib/client-address.ts`) derives, or removed when it derives none; the backend keys every per-IP rate limit on it. The address is read from the RIGHT of the chain, `TRUSTED_PROXY_HOPS` (default 1) entries from the end, because every edge appends its peer and a client's own entries land on the left; `X-Real-IP` or any other dedicated header is believed only when named in `CLIENT_IP_HEADER`, since Traefik and Envoy pass a client-sent one through. Reading the first entry, or `X-Real-IP` by default, let any client choose its own rate-limit bucket.
- **Request bodies:** Next copies the body of every request the proxy matches into memory before the proxy runs, up to `experimental.proxyClientMaxBodySize` in `next.config.js`, so that setting is the most one unauthenticated request can pin and it stays small (11 MB). The proxy refuses any body over the backend's 10 MB default with 413 (`readBoundedBody`). The few routes that need more (`LARGE_UPLOAD_ROUTES` in `src/lib/proxy-body-limit.ts`: the `.mny` import, the backup restore, the assistant's query, transaction attachments) are excluded from `config.matcher` and served by route handlers under `src/app/api/v1/` that stream the body to the backend with a per-route ceiling (`forwardLargeUpload`). A new large-body route is added in all three places; `proxy-matcher.test.ts` and `large-upload-proxy.test.ts` check they agree.
