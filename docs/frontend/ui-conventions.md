...
The switch is drawn as two icons, not two words, and the words survive as each button's `aria-label` and `title`. It sits in one corner of the screen in both views: the right-hand end of `CalendarToolbar`'s row, grouped with the legend behind one `sm:ml-auto` (two `ml-auto` siblings would SPLIT the free space between them rather than both moving right), and in table mode the last of `ListTopToolbar`'s buttons -- immediately left of the pager and immediately right of the density toggle, so the two controls that change what the whole list looks like sit together. Neither component renders it; each takes it as a `viewToggle` node, because which surface it switches is the page's business. On the Investments page `InvestmentTransactionList` keeps `viewToggle` for the brokerage/cash switch beside its heading and takes the Table/Calendar one as `viewModeToggle`, which only reaches the strip when that list owns its paging.
