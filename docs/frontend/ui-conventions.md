...
Below `sm` a cell is the day number, a dot row coloured like the chips and a count, because a phone cell has no room for a label and a row of dots cannot say how many; below `lg` the day panel is a `Modal` over the month rather than a card beside it, since there is no column to put it in. Both are decided by `useIsBelowDesktop` / `useIsMobile` (`hooks/useIsMobile.ts`, one `useMediaQuery` under both), never by a second `matchMedia` call.
