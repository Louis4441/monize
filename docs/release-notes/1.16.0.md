...
- The **Transactions** account info widget gains a caret that opens the account switcher, so you can change account without going back to the list. Your starred accounts sit at the top of it under a **Favourites** heading, in the order you arranged them, with everything else by name below -- the same order every account dropdown in the app already used. Typing in the filter still reaches both sections. The menu is drawn above everything and clamped to the viewport, and on a touch device it opens without the keyboard leaping up.
