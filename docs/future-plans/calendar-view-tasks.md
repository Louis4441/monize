...
Implement design section 6.3. The trading-day query reuses `getLastPricedDays`'s held-securities subquery, matched on the exact day; do not add a second replay: the detail endpoint reads quantities through the same `applyActionToQuantity` replay and closes through `positionCloseAsOf` (INV-HOLDING-002).
