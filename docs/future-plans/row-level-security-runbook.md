...
| `app.bypass_rls` | `'on'` / unset | Privileged: see across all users | `withScopedDb()`, transaction-locally, inside a `withSystemContext()` scope only (import lint-allowlisted; invocations logged with call site) |
