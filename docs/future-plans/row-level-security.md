...
> Enumeration corrected from an earlier draft, which wrongly listed `account_delegates`, `emergency_access_*`, and `oauth_payloads` here (none has a `user_id` column -- the templated policy would fail at migration time with `column "user_id" does not exist` and crash-loop startup) and listed `budget_*` as a family (only `budgets` and `notifications` -- then still named `budget_alerts`, renamed by migration 172 -- are direct; `budget_categories`, `budget_periods`, `budget_period_categories` are indirect). **M2's authoring step must re-verify every table against `schema.sql`, not this list.**
```sql
