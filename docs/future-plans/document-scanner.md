...
| I6 | **A scan result belongs to the request that produced it.** A stale worker reply (from a previous photo, or after Retake) is dropped. | Every worker message carries a request id; the hook adopts a reply only when its id matches the current request (`docs/frontend/api-and-cache.md`, asynchronous data rule). |
