...
| `JWT_SECRET_WEAK` | warning | admins | the 15-minute sweep, while `jwtSecretWeakness` flags `JWT_SECRET` (a published placeholder or a typed pattern that is long enough to boot); `data.reason` is `placeholder` or `predictable`, never the secret | `JWT_SECRET_WEAK:<iso-week>` | yes |
