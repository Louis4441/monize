 ...
 *
 * Wins the first claim on a `purpose:token` pair and loses every later one,
 * which is the real service's whole contract. A double that always returned
...
export type SingleUseTokenMock = jest.Mocked<
  Pick<SingleUseTokenService, "claim">
> & {
