  ...
  register(server: McpServer, resolve: UserContextResolver) {
    server.registerTool(
      "list_upcoming_bills",
      {
        ...
        description:
          "Get upcoming scheduled bills and deposits due within a date window. Each item is classified as bill / deposit / transfer / investment and includes a daysUntilDue value (negative when overdue). Each item is ONE occurrence -- the next one due -- so `nextDueDate` is the date that occurrence actually falls on (a per-occurrence override can move it off the schedule's own recurrence date) and `amount` is what that occurrence would post today in its own `currency`, and is null with `amountComplete` false when the current exchange rate for it cannot be determined -- report such an item as unknown rather than guessing. A bucket total (`totalUpcomingBills` / `totalUpcomingDeposits`) is null for the same reason, with the partial sum in `knownUpcomingBillsSubtotal` / `knownUpcomingDepositsSubtotal`; quote that only as a subtotal. `amountsComplete` and `unknownAmountItems` name the shortfall. Returns the same shape as the AI Assistant's list_upcoming_bills tool.",
        inputSchema: {
          days: z
            .number()
            .min(1)
