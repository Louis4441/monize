...
export const FINANCIAL_TOOLS: AiToolDefinition[] = [
  ...
  },
  {
    ...
    description:
      ...
      "split transactions (create or update): add a 'splits' array of { categoryName, amount, memo? } (>= 2 lines, category splits only) instead of a single categoryName; the split amounts must sum to the transaction amount (e.g. a -100 expense split -60 Groceries / -40 Household). On create also give accountName, amount, date; on update give transactionId and splits (replaces the whole split set). Updating an EXISTING split transaction: parent fields (date, payeeName, description) work without splits, but changing its categories or its amount requires resending the COMPLETE splits array (the tool errors with instructions otherwise). Read tools return one row per split line (same transaction id, distinct splitId), so read the current lines first, then resend all lines with your change applied. Send split transactions one at a time (a single item), not mixed into a multi-row batch. Only one proposing tool call is allowed per reply, so several split transactions CANNOT be proposed in one answer: propose the first, then tell the user plainly how many remain and that they can ask you to continue with the next. Never promise cards for transactions you have not proposed. " +
