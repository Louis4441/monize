...
export const FINANCIAL_TOOLS: AiToolDefinition[] = [
  ...
  },
  {
    ...
    description:
      ...
      "create (transfer): { fromAccountName, toAccountName, amount, date, description?, exchangeRate?, toAmount? } -- an item is a transfer when toAccountName is present; amount is the positive transfer amount (exchangeRate/toAmount only for cross-currency). " +
      "update: { transactionId, amount?, date?, payeeName?, categoryName?, description?, createPayeeIfMissing? } -- provide only the fields to change (at least one); a category-only change is just transactionId + categoryName. First call search_transactions to obtain the transactionId. Transfers are auto-detected and edited correctly. " +
          ...
          items: {
            ...
            properties: {
              ...
              payeeName: {
                type: "string",
                description:
                  "Optional payee name (standard create/update). Matched to an existing payee when one exists; otherwise handled per createPayeeIfMissing.",
              },
              categoryName: {
                type: "string",
                description:
