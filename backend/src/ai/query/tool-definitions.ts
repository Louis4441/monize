...
export const FINANCIAL_TOOLS: AiToolDefinition[] = [
  ...
  },
  {
    ...
    description:
      "Get upcoming scheduled bills and deposits due within a date window. Each item is classified as bill (scheduled outflow), deposit (scheduled inflow), transfer, or investment, and includes a daysUntilDue value (negative when overdue). Each item is ONE occurrence -- the next one due -- so `nextDueDate` is the date that occurrence actually falls on (a per-occurrence override can move it off the schedule's own recurrence date) and `amount` is what that occurrence would post today in its own `currency`; it is null with `amountComplete` false when the current exchange rate behind it cannot be determined, and you must report such an item as unknown rather than guessing a figure. Returns rollup totals for upcoming bills and deposits plus the per-item list -- a total is null when any item in that bucket is unknown, with the partial sum in `knownUpcomingBillsSubtotal` / `knownUpcomingDepositsSubtotal`, which you may quote only as a subtotal. `amountsComplete` and `unknownAmountItems` say whether anything is missing and which items. Use for questions like 'what bills are coming up', 'when is rent due', or 'what deposits am I expecting this month'.",
    inputSchema: {
      type: "object",
      properties: {
        days: {
