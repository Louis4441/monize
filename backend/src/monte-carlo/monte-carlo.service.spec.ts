...
describe("MonteCarloService", () => {
  ...
  describe("duplicate", () => {
    it("creates a copy with cash flows and resets metadata", async () => {
      const source = buildScenario({
        ...
        ] as MonteCarloCashFlow[],
      });
      scenariosRepository.findOne
        .mockResolvedValueOnce(source)
        .mockResolvedValueOnce(buildScenario({ id: "scn-2", name: "Copy of Retirement" }));
      scenariosRepository.save.mockResolvedValueOnce({ id: "scn-2" });

      const result = await service.duplicate(userId, "scn-1");

      expect(scenariosRepository.create).toHaveBeenCalledWith(
        expect.objectContaining({
          userId,
          name: "Copy of Retirement",
          isFavourite: false,
          lastRunAt: null,
        }),
      );
      ...
      expect(cashFlowsRepository.save).toHaveBeenCalled();
      expect(result.id).toBe("scn-2");
    });

    it("throws NotFound when the source scenario is missing", async () => {
      scenariosRepository.findOne.mockResolvedValueOnce(null);
      await expect(service.duplicate(userId, "scn-1")).rejects.toBeInstanceOf(
        NotFoundException,
      );
    });

    it("does not create cash flow rows when source has none", async () => {
      const source = buildScenario({ cashFlows: [] });
      scenariosRepository.findOne
        .mockResolvedValueOnce(source)
        .mockResolvedValueOnce(buildScenario({ id: "scn-2" }));
      scenariosRepository.save.mockResolvedValueOnce({ id: "scn-2" });

      await service.duplicate(userId, "scn-1");
