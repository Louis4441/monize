...
**A feature that is on by default cannot hang off optional configuration.** The stored password was encrypted with `AI_ENCRYPTION_KEY`, which is commented out in `.env.example`, documented as being for cloud AI providers, and passed as `${AI_ENCRYPTION_KEY:-}` by the compose files -- so on a deployment that configured no AI provider the capture returned early and every automatic backup was written in plaintext, with no log line, no status field and nothing on the Settings page (issue #1269). `BackupPasswordCipher` derives its key from `JWT_SECRET`, which startup enforces, via `derivePurposeKey` (the same domain separation TOTP secrets use); `AI_ENCRYPTION_KEY` stays preferred where one is set, and both keys are accepted on read so adding or removing it strands nothing. Two rules come out of it, and both have tests: a plaintext automatic backup is **logged** on every write (`auto-backup.service.ts`), and "this server cannot encrypt" is a distinct field in `getStatus` from "this user has not enabled it", because they have different fixes. A service spec whose cipher double answers `isConfigured() === true` is structurally blind to this class of bug -- `backup-encryption.service.spec.ts` therefore ends with a block that builds the **real** cipher over an environment holding only `JWT_SECRET`.
